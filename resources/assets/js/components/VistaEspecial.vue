...
export default {
  ...
  created() {
  this.nombreRama = this.$route.query.nombre ? this.$route.query.nombre.replace(/^\d+\.\s*/, '') : 'Nombre de rama no disponible';
  console.log("Nombre Rama: ", this.nombreRama);
  this.ramaId = this.getRamaId(this.nombreRama);
  console.log("Rama ID: ", this.ramaId);
  },
  ...
  methods: {
    ...
    async getRecurso() {
      try {
        ...
        console.log("Llamando a la API con tema_rama:", this.ramaId);

        // Realiza la solicitud a la API pasando el parámetro de tema_rama
        const result = await axios.get('/recursosvistaespecial/mostrarrecursosvistaespecial', {
          params: { tema_rama: this.ramaId }  // Envía el valor del ID de la rama
        });
