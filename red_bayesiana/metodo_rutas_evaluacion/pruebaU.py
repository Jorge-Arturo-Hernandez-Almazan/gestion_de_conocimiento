...
print(f"Buscando archivos para la matrícula: {matricula}")  # Log adicional
...
if not os.path.isdir(carpeta_destino):
    print(f"Error: La carpeta de destino no existe: {carpeta_destino}")
    sys.exit(1)
...
if not archivos_encontrados:
    ...
    print(f"Archivos encontrados: {archivos_encontrados}")  # Log adicional
    for archivo in archivos_encontrados:
        try:
            # Definir la ruta completa del archivo de origen
            ruta_origen = os.path.join(carpeta_origen, archivo)
            print(f"Ruta de origen del archivo: {ruta_origen}")  # Log adicional

            # Crear la carpeta con el nombre del archivo en rbma2 si no existe
            carpeta_archivo = os.path.join(carpeta_destino, os.path.splitext(archivo)[0])
            ...
            print(f"Ruta de destino del archivo: {ruta_destino}")  # Log adicional
