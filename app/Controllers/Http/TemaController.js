...
class TemaController {
    ...
    async deleteNodoSecundarioRelation({
        response,
        request
    }) {
        ...
        const {
            ...
            id_hijo,
            tipo
        } = request.only(['id_padre', 'id_hijo', 'tipo'])
    ...
    async agregarTema({
        ...
        const temaId = await Database.insert({
    ...
    async registrar({
        ...
        const relacion = await Database.insert({
            id_padre: id_padre,
            id_hijo: dametema,
            tipo: 'primarias',
            color: color
        }).into('relacion_primarias')
        return response.json({
            message: 'Creado',
            newId: temaId,
            xd: estotrae_freey,
    ...
    async posicion({
        ...
        await Tema.query().where('id', id).update({
            freex: newx,
            freey: newy
        })
    ...
    async temacount({
        ...
        const tema = await Database.raw('SELECT COUNT(id) as totalTemas FROM temas')
        return response.json(tema)
    }


    async posiblesNodosPadres({
        response,
        params
    }) {
